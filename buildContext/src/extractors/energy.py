    ...
    def extraction(self, query_strings):
        ...
        try:
            ...
            if control_area == "nyiso" or control_area == "miso" or control_area == "ercot" or control_area == "pjm" or control_area == "isone":
                data_frame = None
                psql_query = f"select * from trueprice.{control_area}_energy where (strip = '{strip.lower()}' or '{strip.lower()}' = 'all' ) and month::date >= '{start_date}' and month::date <= '{end_date}';"
                print(psql_query)
                data_frame = pd.read_sql_query(sql=psql_query, con=self.engine.connect())
                return data_frame, "success"  
            else:
                return None, f"Unable to fetch results, invalid control area: {control_area}"
