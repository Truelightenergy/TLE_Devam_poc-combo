...
class Miso_Energy:
    ...
    def ingestion(self, data):
        """
        Handling Ingestion for energy for miso
        """
            ...
            with self.engine.connect() as con:
                ...
                backup_query = f'''
                    with current as (
                        ...
                        select id, strip, curvestart, amilcips_amount, amilcilco_amount, amilip_amount, indy_amount from trueprice.{data.controlArea}_energy where curvestart>='{sod}' and curvestart<='{eod}' and strip='{data.strip}'
                    ),
                    backup as (
