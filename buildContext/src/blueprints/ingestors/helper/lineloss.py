...
class Line_Loss:
    ...
    def ingestion(self, data):
        try:
            ...
            check_query = f"""
                ...
                select exists(select 1 from trueprice.{data.curveType} where curvestart='{now}') -- ignore, "db == file" based on timestamp
                UNION ALL
                select exists(select 1 from trueprice.{data.curveType} where curvestart>='{sod}' and curvestart<='{now}') -- update, db already exists
                UNION ALL
                select exists(select 1 from trueprice.{data.curveType} where curvestart>='{now}' and curvestart<'{eod}' ) -- ignore, db is equal or newer
            """
            ...
            if same: # if data already exists neglect it
                ...
                r = df.to_sql(f"{data.curveType}", con = self.db_util.engine, if_exists = 'append', chunksize=1000, schema="trueprice", index=False)
                if r is not None:
                    return "Data Inserted"
                return "Insert aborted, failed to insert new data"
                    
            elif old_exists: # perform scd-2
                tmp_table_name = f"{data.curveType}_{data.snake_timestamp()}" # temp table to hold new csv data so we can work in SQL
                ...
                with self.db_util.engine.connect() as con:
                    ...
                    backup_query = f'''
                        ...
                        -- update the existing "current" with the new "csv"
                            ...
                            deletion as(
                            DELETE from trueprice.{data.curveType}
    ...
    def setup_dataframe(self, data_frame, curveType):
        ...
        try:
            ...
            temp_time = time.time()
            hierarchy = self.hierarchy.get_hierarchy_id(data_frame, curveType)
