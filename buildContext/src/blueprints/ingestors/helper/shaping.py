...
class Shaping:
    ...
    def ingestion(self, data):
        try:
            ...
            temp_time = time.time()
            df= self.setup_dataframe2(df, data.curveType)
            ...
            #     UNION ALL
            #     select exists(select 1 from trueprice.{data.controlArea}_{data.curveType} where curvestart>='{sod}' and curvestart<'{eod}' and cob ) -- ignore, db has cob already
            # """
            check_query = f"""
            select exists(select 1 from trueprice.{data.controlArea}_{data.curveType} where curvestart='{now}') -- ignore, "db == file" based on timestamp
            """
            ...
            #                     insert into trueprice.{data.controlArea}_{data.curveType}_history ( cob, month, curvestart, curveend, data, control_area, state, load_zone, capacity_zone, utility, strip, cost_group, cost_component, sub_cost_component)
            ...
            #                 updation as (
            #                 insert into trueprice.{data.controlArea}_{data.curveType} ( cob, month, curvestart, data, control_area, state, load_zone, capacity_zone, utility, strip, cost_group, cost_component, sub_cost_component)
    ...
    def setup_dataframe2(self, data_frame, curveType):
        """
        formats the data frame into proper format
        """
        try:
