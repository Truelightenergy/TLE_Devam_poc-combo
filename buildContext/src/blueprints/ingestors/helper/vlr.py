...
class Vlr:
                    ...
                    backup_query = f'''
                        ...
                        -- insertion to the database in history table
                            with current as (
                                ...
                                select id, curvestart, data, hierarchy_id, he, datemonth
                                from trueprice.{data.controlArea}_{data.curveType} where curvestart>='{sod}' and curvestart<='{eod}'
                            ),
                            backup as (
                                -- take current rows and insert into database but with a new "curveend" timestamp

                                insert into trueprice.{data.controlArea}_{data.curveType}_history (curvestart, curveend, data, hierarchy_id, he, datemonth)

                                select curvestart, '{curveend}' as curveend, data, hierarchy_id, he, datemonth
                                from current
                            ),
                            single as (
                        ...
                        -- update the existing "current" with the new "csv"
                            ...
                            updation as (
                            insert into trueprice.{data.controlArea}_{data.curveType} ( curvestart, data, hierarchy_id, he, datemonth)

                            select  curvestart, data, hierarchy_id, he, datemonth
    ...
    def setup_dataframe(self, data_frame, curveType):
        ...
        try:
            ...
            melted_df.rename(columns={0:"datemonth", 1:'he'}, inplace=True)
