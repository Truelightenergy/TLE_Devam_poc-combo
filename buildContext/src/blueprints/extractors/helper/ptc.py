        ...
        try:
            ...
            if control_area not in ["nyiso", "miso", "ercot", "pjm", "isone"]:
                ...
                if query_strings["history"]:
                    psql_query = f"""
                        select id, month, curvestart, TO_TIMESTAMP('9999-12-31 23:59:59','YYYY-MM-DD HH24:MI:SS') as curveend, data, control_area, state, load_zone, capacity_zone, utility, strip, cost_group, cost_component, lookup_id2, lookup_id3, control_area_type, utility_name, profile_load from trueprice.ptc 
                        where control_area_type = '{control_area}' and ({strip_query}) and month::date >= '{start_date}' and month::date <= '{end_date}'
                        UNION
                        select id, month, curvestart, curveend, data, control_area, state, load_zone, capacity_zone, utility, strip, cost_group, cost_component, lookup_id2, lookup_id3, control_area_type, utility_name, profile_load from trueprice.ptc_history
                        where control_area_type = '{control_area}' and ({strip_query}) and month::date >= '{start_date}' and month::date <= '{end_date}'
