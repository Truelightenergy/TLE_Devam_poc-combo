            ...
            if control_area not in ["nyiso", "miso", "ercot", "pjm", "isone"]:
                ...
                if query_strings["history"]:
                    psql_query = f"""
                        select id, month, curvestart, TO_TIMESTAMP('9999-12-31 23:59:59','YYYY-MM-DD HH24:MI:SS') as curveend, control_area_type, control_area, state, load_zone,
                                    capacity_zone, utility, strip, cost_group, cost_component, load_profile, headroom, headroom_prct, ptc, lookup_id2, lookup_id3, term from trueprice.headroom 
                        ...
                        select id, month, curvestart, curveend, control_area_type, control_area, state, load_zone,
                                    capacity_zone, utility, strip, cost_group, cost_component, load_profile, headroom, headroom_prct, ptc, lookup_id2, lookup_id3, term from trueprice.headroom_history
                        where control_area_type = '{control_area}' and ({strip_query}) and month::date >= '{start_date}' and month::date <= '{end_date}'
                ...
                if query_strings["history"]:
                    psql_query = f"""
                        ...
                        UNION
                        select id, month, curvestart, curveend, control_area_type, control_area, state, load_zone,
                                    capacity_zone, utility, strip, cost_group, cost_component, load_profile, headroom, headroom_prct, ptc, lookup_id2, lookup_id3, term, term from trueprice.headroom_history
                        where control_area_type = '{control_area}' and ({strip_query}) and month::date >= '{start_date}' and month::date <= '{end_date}'
